...
namespace BaselineSolution.Framework.Infrastructure.Including
{
    public class EntityIncluder<TEntity> : IEntityIncluder<TEntity> where TEntity : class
    {
        ...
        public IEntityIncluder<TCast> Cast<TCast>() where TCast : class, TEntity
        {
            if (_baseIncluder != null)
            {
                return new EntityIncluder<TCast>(_baseIncluder.Cast<TCast>(), _propertyInclusionHolder.Cast<TCast>());
            }
            return new EntityIncluder<TCast>(null, _propertyInclusionHolder.Cast<TCast>());
        }
