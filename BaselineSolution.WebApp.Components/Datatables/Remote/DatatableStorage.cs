...
namespace BaselineSolution.WebApp.Components.Datatables.Remote
{
    ...
    public static class DatatableStorage
    {
        ...
        public static RemoteDatatable<TEntity> Get<TEntity>(string datatableId, Action datatableRenderer) where TEntity : class 
        {
            if (!Has(datatableId))
                datatableRenderer();
            if (!Has(datatableId))
        ...
        public static void Put<TEntity>(RemoteDatatable<TEntity> datatable) where TEntity : class 
        {
